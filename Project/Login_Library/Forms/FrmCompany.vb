Public Class FrmCompany
    ...
    Private Sub FSelectCompany()
        ...
        If FGMain(GCompanyCode, FGMain.CurrentRow.Index).Value <> "" Then
            ...
            If AgL.StrCmp(bCompanyName.Substring(0, 18), "Kanpur Consultants") Then
                Academic_ProjLib.ClsMain.IsClient_KC = True
                AgL.PubKillerDate = ""
            ElseIf AgL.StrCmp(bCompanyName.Substring(0, 6), "K.P.S.") Then
                Academic_ProjLib.ClsMain.IsClient_KPS = True
                AgL.PubKillerDate = "21/Jan/2013"
            End If
            ...
            If Trim(AgL.PubSiteCode) = "" Then
                mQry = "SELECT IsNull(Count(*),0) Cnt, Max(S.Code) AS Site_Code " & _
                        " FROM SiteMast S " & _
                        " WHERE IsNull(S.Active,0) <> 0"
                DtTemp = AgL.FillData(mQry, AgL.GCn).Tables(0)
                If DtTemp.Rows.Count > 0 Then
                    If DtTemp.Rows(0).Item("Cnt") = 1 Then
            ...
            'If Academic_ProjLib.ClsMain.IsClient_GlobalGroup Then
            '    If AgL.StrCmp(AgL.PubSiteCodeDisplay, "GLOBAL GROUP") Then
            '        mQry = "Select Count(*) From Sch_Student"
            '        If AgL.Dman_Execute(mQry, AgL.GCn).ExecuteScalar > 200 Then
            '            MsgBox("File sharing lock count exceeded. Increase MaxLocksPerFile registry entry.", MsgBoxStyle.Critical, "System Error")
            '            End
            '        Else
            '            MsgBox("FCB::Open failed: Could not open file Master.mdf for file number 0.  OS error: 5(Access is denied.).", MsgBoxStyle.Critical, "System Error")
            '            End
            '        End If
            '    End If
            'End If
            ...
            FAddMenu(MD, mCommon_Master)
            MD.StrCurrentModule = mCommon_Master
            MD.Show()
            Me.Dispose()
        End If
    End Sub
    ...
    Private Sub ProcActiveModule()
        'Academic_ProjLib.ClsMain.IsModuleActive_CommonMaster = True
        Academic_ProjLib.ClsMain.IsModuleActive_FinancialAccount = True
        Academic_ProjLib.ClsMain.IsModuleActive_Utility = True
        AgLibrary.ClsConstant.IsOldFaVoucherEntryActive = False

        If Academic_ProjLib.ClsMain.IsClient_KC Then
            AgLibrary.ClsConstant.IsSmsActive = True
            AgLibrary.ClsConstant.IsEMailActive = True
            AgLibrary.ClsConstant.BlnIsAuditFaVoucher = True

            Academic_ProjLib.ClsMain.IsModuleActive_AcademicMain = True
            Academic_ProjLib.ClsMain.IsModuleActive_FeeModule = True
            Academic_ProjLib.ClsMain.IsModuleActive_TimeTable = True
            Academic_ProjLib.ClsMain.IsModuleActive_Exam = True
            Academic_ProjLib.ClsMain.IsModuleActive_Store = True
            Academic_ProjLib.ClsMain.IsModuleActive_Hostel = True
            ...
            Academic_ProjLib.ClsMain.IsModuleActive_Canteen = True

        ElseIf Academic_ProjLib.ClsMain.IsClient_KPS Then
            AgLibrary.ClsConstant.IsSmsActive = True
            AgLibrary.ClsConstant.IsEMailActive = True
            AgLibrary.ClsConstant.BlnIsAuditFaVoucher = True
