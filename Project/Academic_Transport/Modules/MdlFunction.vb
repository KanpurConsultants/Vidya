...
Module MdlFunction
    ...
    Public Function FunIsScholarVersion() As Boolean
        Dim bBlnReturn As Boolean = False
        Dim mQry As String = ""

        Try
            If AgL.IsFieldExist("IsScholar", "Company", AgL.GCn) Then
                mQry = "Select C.IsScholar From Company C With (NoLock) Where C.Comp_Code ='" & AgL.PubCompCode & "'"
                bBlnReturn = AgL.VNull(AgL.Dman_Execute(mQry, AgL.GcnRead).ExecuteScalar)
        ...
        FunIsScholarVersion = bBlnReturn
    ...
    Public Sub IniDtTp_Enviro()
        If AgL.IsTableExist("Tp_Enviro", AgL.GCn) Then
            DtTp_Enviro = AgL.FillData("SELECT E.* FROM Tp_Enviro E WHERE E.Site_Code ='" & AgL.PubSiteCode & "'", AgL.GCn).Tables(0)
        End If
    End Sub
