...
Public Class FrmSiteMaster
    ...
    Sub Ini_List()
        ...
        mQry = "Select SubCode As Code, Name  From ViewSubGroup " & _
            "Where IsNull(CommonAc,0)<> 0 And Left(MainGrCodeS," & AgLibrary.ClsMain.PubBranchDivisionsMainGRLen & ")='" & AgLibrary.ClsMain.PubBranchDivisionsMainGRCode & "' " & _
            "Order By CityName "
    ...
    Private Sub Topctrl1_tbSave() Handles Topctrl1.tbSave
        ...
        Try
            ...
            If Topctrl1.Mode = "Add" Then
                mQry = "Insert Into SiteMast (" & _
                        " Code, ManualCode, Name, HO_YN, Add1, Add2, Add3, City_Code, PinNo, " & _
                        " Phone, Mobile, AcCode, CreditLimit, Active, EMail, WebSite,IsLibrary, " & _
                        " U_EntDt, U_Name, U_AE) Values(" & _
                        " '" & mSearchCode & "', " & AgL.Chk_Text(TxtManualCode.Text) & ", " & AgL.Chk_Text(CboName.Text) & ", " & _
                        " '" & AgL.MidStr(TxtHO_YN.Text, 0, 1) & "', " & AgL.Chk_Text(TxtAdd1.Text) & "," & _
                        " " & AgL.Chk_Text(TxtAdd2.Text) & ", " & AgL.Chk_Text(TxtAdd3.Text) & "," & _
                        " " & AgL.Chk_Text(CboCity_Code.SelectedValue) & ", " & AgL.Chk_Text(TxtPinNo.Text) & "," & _
                        " " & AgL.Chk_Text(TxtPhone.Text) & ", " & AgL.Chk_Text(TxtMobile.Text) & ", " & AgL.Chk_Text(TxtAcCode.AgSelectedValue) & ", " & _
                        " " & Val(TxtCreditLimit.Text) & ", " & IIf(AgL.StrCmp(TxtActive.Text, "Yes"), 1, 0) & "," & _
                        " " & AgL.Chk_Text(TxtEMail.Text) & ", " & _
                        " " & AgL.Chk_Text(TxtWebSite.Text) & "," & IIf(AgL.StrCmp(TxtIsLibrary.Text, "Yes"), 1, 0) & ", " & _
                ...
                If AgL.PubSitewiseV_No = True Then
                    ...
                    mQry = "Delete From Voucher_Prefix Where Site_Code='" & mSearchCode & "' And Div_Code='" & AgL.PubDivCode & "' And IsNull(Comp_Code,'" & AgL.PubCompCode & "') = '" & AgL.PubCompCode & "' "
                    AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)

                    mQry = "Insert Into Voucher_Prefix(Site_Code,V_Type,Date_From,Date_To,Prefix,Start_Srl_No,Div_Code,Comp_Code)" & _
                            " (Select '" & mSearchCode & "' As Site_Code,V_Type,Date_From,Date_To,Prefix,0 As Start_Srl_No,'" & AgL.PubDivCode & "' As Div_Code, '" & AgL.PubCompCode & "' As Comp_Code From Voucher_Prefix Where Site_Code='" & AgL.PubSiteCode & "' And Div_Code='" & AgL.PubDivCode & "' And IsNull(Comp_Code,'" & AgL.PubCompCode & "') = '" & AgL.PubCompCode & "' )"
                    AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
                    ...
                    mQry = "Insert Into Enviro(Id,Site_Code,Div_Code, " & _
                            ...
                            " '" & mEnviroId & "', '" & mSearchCode & "', '" & AgL.PubDivCode & "', " & _
                            " '" & Format(AgL.PubLoginDate, "Short Date") & "', '" & AgL.PubUserName & "', 'A') "
                    AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)
                    ...
                    mQry = "INSERT INTO FaEnviro ( " & _
                            ...
                            " RunPIF, FilterAC, DateLock, AddressHelp, CashBookBalance, MonthTotal, OnLineAdjustment, " & _
                            " OpStockQTY, OpStockValue, ClStockQTY, ClStockValue, CashBookPage, RepToBy, PreBal, " & _
                            " PDCDt, ToBy, CityNameDisp, '" & mSearchCode & "' As Site_Code " & _
                            " From FaEnviro Where Site_Code='" & AgL.PubSiteCode & "') "
                    AgL.Dman_ExecuteNonQry(mQry, AgL.GCn, AgL.ECmd)

                End If
            Else
                mQry = "Update SiteMast Set ManualCode=" & AgL.Chk_Text(TxtManualCode.Text) & ", Name = " & AgL.Chk_Text(CboName.Text) & ", HO_YN = '" & AgL.MidStr(TxtHO_YN.Text, 0, 1) & "'," & _
                        " Add1 = " & AgL.Chk_Text(TxtAdd1.Text) & ", Add2 = " & AgL.Chk_Text(TxtAdd2.Text) & ", Add3 = " & AgL.Chk_Text(TxtAdd3.Text) & "," & _
                        " City_Code = " & AgL.Chk_Text(CboCity_Code.SelectedValue) & ", PinNo = " & AgL.Chk_Text(TxtPinNo.Text) & "," & _
                        " Phone = " & AgL.Chk_Text(TxtPhone.Text) & ", Mobile = " & AgL.Chk_Text(TxtMobile.Text) & ", AcCode = " & AgL.Chk_Text(TxtAcCode.AgSelectedValue) & ", CreditLimit = " & Val(TxtCreditLimit.Text) & "," & _
                        " Active = " & IIf(AgL.StrCmp(TxtActive.Text, "Yes"), 1, 0) & ", " & _
                        " EMail = " & AgL.Chk_Text(TxtEMail.Text) & ", " & _
                        " WebSite = " & AgL.Chk_Text(TxtWebSite.Text) & ", " & _
                        " IsLibrary = " & IIf(AgL.StrCmp(TxtIsLibrary.Text, "Yes"), 1, 0) & ", " & _
            ...
            If mSearchCode <> "" Then
                ...
                With DsTemp.Tables(0)
                    If .Rows.Count > 0 Then
                        ...
                        TxtIsLibrary.Text = IIf(AgL.VNull(.Rows(0)("IsLibrary")), "Yes", "No")
    ...
    Private Function Data_Validation() As Boolean
        Try
            ...
            If Not AgL.IsValid_EMailId(TxtEMail, LblEMail.Text) Then Exit Function
            ...
            If TxtHO_YN.Text = "Yes" Then
                mQry = "Select IsNull(Count(*),0) As Cnt From SiteMast Sm Where IsNull(Sm.Ho_Yn,'')='Y' " & IIf(Topctrl1.Mode = "Add", "", " And Code<>'" & mSearchCode & "'") & " "
