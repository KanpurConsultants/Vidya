...
#Region "Form Designer Code"
    Private Sub InitializeComponent()
        ...
        Me.LblNetAmount.Size = New System.Drawing.Size(76, 16)
        ...
        Me.LblPaidAmount.Size = New System.Drawing.Size(82, 16)
        ...
        Me.LblCashBankAc.Size = New System.Drawing.Size(95, 16)
        ...
        Me.Label1.Location = New System.Drawing.Point(517, 121)
...
#End Region
    ...
    Private Sub FrmSaleOrder_BaseEvent_Save_InTrans(ByVal SearchCode As String, ByVal Conn As System.Data.SqlClient.SqlConnection, ByVal Cmd As System.Data.SqlClient.SqlCommand) Handles Me.BaseEvent_Save_InTrans
        ...
        For I = 0 To Dgl1.RowCount - 1
            If Dgl1.Item(Col1ReferenceDocID, I).Value <> "" And AgL.StrCmp(Dgl1.Item(Col1Select, I).Value, AgLibrary.ClsConstant.StrCheckedValue) Then
                mSr += 1
                mQry = "Insert Into DuesPaymentDetail_Log( " & _
                        "UID, " & _
                        "DocId, " & _
                        "Sr, " & _
                        "TransactionType, " & _
                        ...
                        " " & mSr & ", " & _
                        " " & AgL.Chk_Text(IIf(TransType = TransactionType.Payment, "Payment", "Receipt")) & ", " & _
                        " " & AgL.Chk_Text(TxtSubCode.AgSelectedValue) & ", " & _
                        " " & Val(TxtCurrBalance.Text) & ", " & _
                        " " & Val(TxtPaidAmount.Text) & ", " & _
                        " " & Val(TxtDiscount.Text) & ", " & _
                        " " & Val(TxtNetAmount.Text) & ", " & _
                        " " & AgL.Chk_Text(TxtCashBank.Text) & ", " & _
                        " " & AgL.Chk_Text(TxtCashBankAc.AgSelectedValue) & ", " & _
                        " " & AgL.Chk_Text(TxtRemarks.Text) & ", " & _
                        " " & AgL.Chk_Text(Dgl1.AgSelectedValue(Col1ReferenceDocID, I)) & ", " & _
                        " " & Dgl1.Item(Col1Ref_Sr, I).Value & ", " & _
                        " " & Val(Dgl1.Item(Col1AdjustedAmt, I).Value) & " )"
                AgL.Dman_ExecuteNonQry(mQry, Conn, Cmd)
            ...
            If .Rows.Count > 0 Then
                ...
                If FrmType = ClsMain.EntryPointType.Main Then
                    ...
                    mQry = " Select Dp.*, IsNull(D." & mDuesAmtFieldName & ",0) AS BillAmount, " & _
                            ...
                            " Where Dp.DocId = '" & SearchCode & "' Order By Dp.Sr"
                Else
                    ...
                    mQry = " Select Dp.*, IsNull(D." & mDuesAmtFieldName & ",0) AS BillAmount, " & _
                            ...
                            " from DuesPaymentDetail_Log Dp " & _
                            " LEFT JOIN dues D ON Dp.ReferenceDocID + Convert(VARCHAR,Dp.Reference_Sr) = D.DocID + Convert(VARCHAR,D.Sr) " & _
                            " Where Dp.UID = '" & SearchCode & "' Order By Dp.Sr"
                End If
                ...
                With DsTemp.Tables(0)
                    ...
                    If .Rows.Count > 0 Then
                        For I = 0 To DsTemp.Tables(0).Rows.Count - 1
                            ...
                            Dgl1.Item(Col1AdjustedAmt, I).Value = Format(AgL.VNull(.Rows(I)("Amount")), "0.00")
                            Dgl1.Item(Col1Ref_Sr, I).Value = AgL.VNull(.Rows(I)("Reference_Sr"))

                            Dgl1.Item(Col1Amount, I).Value = Format(AgL.VNull(.Rows(I)("BillAmount")), "0.00")
                            Dgl1.Item(Col1PaidAmount, I).Value = Format(AgL.VNull(.Rows(I)("PaidAmt")) - AgL.VNull(.Rows(I)("Amount")), "0.00")
                            Dgl1.Item(Col1Balance, I).Value = Format(AgL.VNull(.Rows(I)("BillAmount")) - Val(Dgl1.Item(Col1PaidAmount, I).Value), "0.00")
    ...
    Private Sub FrmSaleOrder_BaseFunction_Calculation() Handles Me.BaseFunction_Calculation
        ...
        mToatlAdjustedAmt = 0
        bTotalAdjustedAmt = Val(TxtNetAmount.Text)
    ...
    Private Sub FrmSaleOrder_BaseEvent_Data_Validation(ByRef passed As Boolean) Handles Me.BaseEvent_Data_Validation
        ...
        If mToatlAdjustedAmt < Val(TxtNetAmount.Text) Then
    ...
    Private Sub ProcFillPendingBills(ByVal bSubCode As String, ByVal bV_Date As String)
        ...
        Try
            If mReferenceNCat <> "" Then
                bConStr &= " And Vt.NCat = '" & mReferenceNCat & "' "
            End If
            ...
            mQry = "SELECT H.DocID, H." & mDuesAmtFieldName & ", H.Sr  , " & _
                    ...
                    " And (IsNull(H." & mDuesAmtFieldName & ",0) - IsNull(H.AdjustedAmount,0) > 0 OR  Dp.DocID = '" & mInternalCode & "') " & bConStr
                ...
                Case TxtSubCode.Name
                    If sender.Text <> "" Then
                        ...
                        mQry = " SELECT IsNull(Sum(H." & mDuesAmtFieldName & "),0) - IsNull(Sum(H.AdjustedAmount),0) " & _
                                " FROM Dues H " & _
                                " LEFT JOIN Voucher_Type Vt On H.V_Type = Vt.V_Type " & _
                                " WHERE H.SubCode = '" & TxtSubCode.AgSelectedValue & "'" & bConStr
