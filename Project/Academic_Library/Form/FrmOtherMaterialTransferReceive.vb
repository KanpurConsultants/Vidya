...
Public Class FrmOtherMaterialTransferReceive
    Inherits AgTemplate.TempStockTransferReceive
    Dim mQry As String = ""
    ...
    Public Sub New(ByVal StrUPVar As String, ByVal DTUP As DataTable)
        ...
        Me.EntryNCat = ClsMain.Temp_NCat.BookTransferReceive
    End Sub
...
#End Region
    ...
    Private Sub FrmOtherMaterialTransferIssue_BaseEvent_Topctrl_tbAdd() Handles Me.BaseEvent_Topctrl_tbAdd
        TxtToGodown.AgSelectedValue = RetDefaultGodown(AgL.PubSiteCode, AgL.PubDivCode, ClsMain.ItemType.Book)
    End Sub
    ...
    Private Sub FrmOtherMaterialTransferReceive_BaseFunction_FIniList() Handles Me.BaseFunction_FIniList
        mQry = "SELECT S.DocID AS Code, Vt.Description + '/' + convert(NVARCHAR,S.V_No)  AS IssueNo, " & _
              ...
              " WHERE Vt.NCat = '" & ClsMain.Temp_NCat.BookTransferReceive & "' "
    ...
    Private Sub FrmProductionOrder_BaseEvent_Topctrl_tbPrn(ByVal SearchCode As String) Handles Me.BaseEvent_Topctrl_tbPrn
        ...
        Try
            ...
            If FrmType = ClsMain.EntryPointType.Main Then
                AgL.PubReportTitle = "Other Material Transfer Receive"
                RepName = "Rug_OtherMaterialTrsnsferReceive_Print" : RepTitle = "Other Material Transfer Receive"
            ...
            Else
                ...
                RepName = "Rug_OtherMaterialTrsnsferReceive_Print" : RepTitle = "Other Material Transfer Receive Log"
                bTableName = "StockHead_Log" : bSecTableName = "Stock_Log  S ON Sh.UID = S.UID "
                bCondstr = "WHERE Sh.UID='" & SearchCode & "'"
            End If
            bCondstr += "And Vt.NCat = '" & ClsMain.Temp_NCat.BookTransferReceive & "' "
            ...
            strQry = "SELECT Sh.DocID, Sh.V_Type, Sh.V_Prefix, Sh.V_Date, Sh.V_No, Sh.Div_Code, " & _
                        ...
                        " Sg.DispName AS StockHeadAccountName, Vc.NCatDescription AS FromProcessDesc, " & _
                        ...
                        " I.Description As ItemDesc, Vc.NCatDescription As StockProcess, " & _
                        " Vt1.Description + '/' + Convert(nVarChar,StockIssue.V_No) As IssurNo, I.ItemGroup " & _
                        ...
                        " LEFT JOIN StockHead StockIssue On S.ReferenceDocId = StockIssue.DocId " & _
                        " LEFT JOIN Voucher_Type Vt1 On StockIssue.V_Type = Vt1.V_Type " & _
                        " " & bCondstr & ""
